...
def plot_density_single(settings, sample_label,
    ...
    for jxn in jxns:
        ...
        if numisoforms > 0:
            ...
            p = PathPatch(a, ec='w', lw=log(jxns[jxn] + 1) /\
    ...
    axvar.spines['right'].set_color('w')
    axvar.spines['top'].set_color('w')
    ...
    if showXaxis:
        ...
        axvar.spines['bottom'].set_color('w')
        xticks([])
...
#                 bar_posterior=False):
    ...
    for sample_num, curr_ax in enumerate(plotted_axes):
        if showYaxis:
            ...
            curr_ax.spines["left"].set_color('white')
            curr_ax.spines["right"].set_color('white')
            curr.ax.set_yticks([])
        ##
        ## Plot sample labels
        ##
